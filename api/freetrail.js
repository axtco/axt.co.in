...
// ✅ FIX: Use destructuring to correctly import the Resend class (resolves "TypeError: Resend is not a constructor")
const { Resend } = require('resend'); 

// --- Configuration ---
// READ VERCEL CUSTOM PREFIXED ENVIRONMENT VARIABLE NAMES (Resolves Vercel reserved names issue)
...
if (!AWS_REGION || !DYNAMODB_TABLE_NAME || !RESEND_API_KEY || !AWS_ACCESS_KEY_ID || !AWS_SECRET_ACCESS_KEY || !SENDER_EMAIL) {
    ...
    // We do not throw here, but the execution will likely fail on client initialization if variables are truly undefined.
}

// Initialize AWS DynamoDB Document Client
const dbClient = new DynamoDBClient({ 
    region: AWS_REGION,
    // Explicitly define credentials using the custom prefixed names
    credentials: {
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // Return a generic error to the client, but log the detail.
