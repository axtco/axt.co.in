...
// --- Configuration ---
// READ NEW VERCEL ENVIRONMENT VARIABLE NAMES
...
if (!AWS_REGION || !DYNAMODB_TABLE_NAME || !RESEND_API_KEY || !AWS_ACCESS_KEY_ID || !AWS_SECRET_ACCESS_KEY) {
    console.error("Missing critical environment variables. Check AXT_AWS_REGION, AXT_AWS_ACCESS_KEY_ID, DYNAMODB_TABLE_NAME, and RESEND_API_KEY.");
}

// Initialize AWS DynamoDB Document Client
// Pass credentials explicitly to bypass Vercel's reserved variable names
 ...
 * Format: AXT-YYYYMMDD-XXXX (e.g., AXT-20251215-0A3F)
 */
 ...
 * @param {object} userData - User data (name, email, specs)
...
`;
    ...
    // HTML content optimized for inbox delivery
    const htmlContent = `
    ...
    try {
        const { error } = await resend.emails.send({
            from: `AXTCloud <${SENDER_EMAIL}>`, // Use the non-spam sender email
            to: userData.email,
            subject: `Your 30-Day Free VPS Trial Request (Ref: ${referenceNumber})`,
            html: htmlContent,
            text: textContent,
            tags: [
                { name: 'category', value: 'free_trial_confirmation' },
            ],
        });

        if (error) {
            console.error('Resend Error:', error);
            // Log the error for debugging, but treat the request as successful for the user
        }
    } catch (err) {
...
module.exports = async (req, res) => {
    ...
    const item = {
        ...
        Status: 'Pending Provisioning', // Initial status
    };
    ...
    try {
        ...
        return res.status(500).json({ error: 'Failed to save request. Database error.' });
    }
